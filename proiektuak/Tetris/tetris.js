...
Tetris.DIRECTION = {'Left':[-1, 0], 'Right':[1, 0], 'Down':[0, 1], 'Behera':[0,1]};
...
Tetris.prototype.do_move = function(direction) {
    ...
    if (this.current_shape.can_move(this.board, dx, dy) && direction!='Behera') {
       // this.clearBoard();
...
Tetris.prototype.do_rotate = function(){
  ...
  var kulunkatu = ["I_Shape","S_Shape","Z_Shape"];
  if(this.current_shape.can_rotate(this.board) && this.current_shape.__proto__.constructor.name != "O_Shape"){
        this.current_shape.rotate();
    if (kulunkatu.indexOf(this.current_shape.__proto__.constructor.name) > -1){
      if(this.current_shape.rotation_dir == 1){
